(function () {
  ...
  network_class.Interface = Class.extend({
    ...
    createFormPrepareCallback: function () {
      ...
      var section = map.section(L.cbi.NamedSection, iface.name(), {
        caption: L.tr('r-internet-setting'),
        anonymous: true
      });

      var pr = section.option(L.cbi.ListValue, 'proto', {
        caption: L.tr('r-connection-type') + L.tr(':')
      });
      ...
      pr.on('change', function (ev) {
        iface.changeProtocol(pr.formvalue(ev.data.sid));
        iface.createForm().insertInto('#map1');
      });
